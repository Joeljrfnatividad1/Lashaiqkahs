...
leakedsb = commands.Bot(description='SELFBOT MADE BY ANONYMOUS',
                           command_prefix='+',
                           self_bot=True,
                           intents=intents)
status_task = None
...
API_URL = 'https://api.kastg.xyz/api/ai/chatgptV4?prompt='
...
async def gpt(ctx, *, prompt: str = None):
    ...
    if prompt is None:
        ...
        await ctx.reply("`-` **PROVIDE A PROMPT**")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        print(data)  # Debug: print the JSON response

                        if data.get("status") == "true":

                            response_text = data['result'][0]['response']

                            await ctx.reply(response_text)

                            print(f"[INFO] GPT prompt processed successfully.")

                        else:

                            await ctx.reply("`-` **API RESPONSE ERROR**")

                            print(f"[ERROR] API response error: {data}")

                    else:

                        await ctx.reply("`-` **API REQUEST FAILED**")

                        print(f"[ERROR] API request failed with status: {resp.status}")
...
# Import commands and listeners from afk.py

from afk import setup

# Setup AFK commands and event listeners
